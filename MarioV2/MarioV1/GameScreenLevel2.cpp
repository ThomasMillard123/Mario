...
bool GameScreenLevel2::SetUpLevel() {
	...
	if (!Layer0->LoadFromFile("Images/MarioBackgroundBig.jpg")) {
...
void GameScreenLevel2::SetLevelMap() {
	...
	for (int y = 0; y < MAP_HEIGHT; y++) {
		...
		for (int x = 0; x < MAP_WIDTH; x++) {
			...
			if (map[y][x] == 2) {
				CreatPowBlock(Vector2D(32 * x, 32 * y));
			}
			if (map[y][x] == 3) {
				CreateCoin(Vector2D(32 * x, 32 * y));
			}
			if (map[y][x] == 4) {
				CreatePipe(Vector2D(32 * x, 32 * y), FACING_RIGHT);
			}
			if (map[y][x] == 5) {
				CreatePipe(Vector2D(32 * x, 32 * y), FACING_LEFT);
			}
			if (map[y][x] == 6) {
				CreateKoopa(Vector2D(32*x, 32*y), FACING_RIGHT, KOOPA_SPEED);
			}
		}
	}

	mNumberOfCoins = mCoins.size();
}
...
void GameScreenLevel2::CreatePipe(Vector2D position,FACING faceingDirection) {
	Pipe* PipeA;
	PipeA = new Pipe(mRenderer, "Images/Pipe.png", position, mLevelMap, faceingDirection);
	mBlocks.push_back(PipeA);
}
void GameScreenLevel2::CreatPowBlock(Vector2D position) {
	mPowBlock = new PowBlock(mRenderer, mLevelMap, position);
}
